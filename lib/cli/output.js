 ...
 * and humans) but can be asked for "json", "tsv", or "ids" formats.
 *
...
/**
 * Collect at most `limit` items from an async iterable, stopping early.
 *
 * Unlike `collect()` (which drains every page), this stops iteration as
 * soon as `limit` items have been gathered, so it never fetches more
 * pages than necessary. Pass `limit = null`/`undefined` to collect all.
 */
export async function take(generator, limit) {
  const out = [];
  if (limit == null) {
    for await (const v of generator) out.push(v);
    return out;
