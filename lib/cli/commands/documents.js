...
export function register(program) {
  ...
  --limit).  Use --all to traverse every page.  For group libraries,
  pass --group <groupId>.  Use --format json for machine-parseable
  ...
  restore the document later.`,
    ...
    .example('mendeley documents delete abcdef12-3456-7890 --yes')
    .action(async ([id], flags, out) => {
      if (!flags.yes) {
        out.fail('Refusing to delete document without --yes.', 2);
        return;
      }
      const session = await buildSession();
      const doc = await session.documents.get(id);
  ...
  text.  Returns the new annotation (text by default; use
  ...
  docs
    .command('deleted')
    .description('list ids of permanently deleted documents (sync primitive)')
    .longDescription(
      `Calls GET /deleted_documents to return the ids of documents that
  have been permanently deleted from the library (or a group, via
  --group). Use --since <iso8601> to fetch only deletions after a
  timestamp — the standard incremental-sync primitive for keeping a
  local cache in sync.

  This is distinct from the trash (soft-deleted documents that can
  still be restored). Combine with --format ids for a clean one-id-
  per-line list.`,
    )
    .option('--since <iso>', 'only deletions after this ISO 8601 timestamp')
    .option('--group <id>', 'query a group library (default: user library)')
    .option('--all', 'fetch every page')
    .option('--limit <n>', 'page size', '500')
    .example('mendeley documents deleted --format ids')
    .example('mendeley documents deleted --since 2024-01-01T00:00:00Z --all')
    .example('mendeley documents deleted --group 12345 --format ids')
    .action(async (_args, flags, out) => {
      const session = await buildSession();
      const resource = flags.group
        ? session.groupDeletedDocuments(flags.group)
        : session.deletedDocuments;
      const kwargs = { pageSize: parseLimit(flags.limit) };
      if (flags.since) kwargs.since = flags.since;
      if (flags.all) {
