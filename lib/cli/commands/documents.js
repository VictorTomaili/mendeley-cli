...
export function register(program) {
  ...
  wins for any colliding keys.`,
    ...
    .action(async (_args, flags, out) => {
      const session = await buildSession();
      let body = {};
      if (flags.data) body = parseJson(flags.data);
      else if (flags.file) body = parseJson(await readFile(flags.file, 'utf8'), '--file');
      else body = {};
  ...
  body as --data (inline JSON) or --file (path to a JSON file).`,
    ...
    .action(async ([id], flags, out) => {
      ...
      const body = flags.data
        ? parseJson(flags.data)
        : flags.file
          ? parseJson(await readFile(flags.file, 'utf8'), '--file')
          : {};
