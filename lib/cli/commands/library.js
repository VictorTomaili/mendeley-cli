...
export function register(program) {
  const cmd = program
    ...
    .longDescription(`These commands combine multiple lower-level API calls into
  ...
  cmd
    ...
    .longDescription(`Walks the entire user library, requests each document with
  ...
  .bib file:
    ...
    .action(async (_args, flags, out) => {
      ...
      const docs = await collect(session.documents.iter({
        view: 'bib',
        pageSize: parseInt(flags.limit, 10),
      }));
      const bib = docs.map(toBibtex).join('\n\n') + '\n';
      if (flags.out) {
        await writeFile(flags.out, bib, 'utf8');
  ...
  cmd
    ...
    .longDescription(`Like \`mendeley library export-bibtex\` but produces JSON.
  ...
  tags/all to control how much metadata is included.`)
    ...
    .action(async (_args, flags, out) => {
      ...
      const json = JSON.stringify(docs.map((d) => d.toJSON()), null, 2);
      if (flags.out) {
  ...
  cmd
    ...
    .longDescription(`Scans the entire user library and reports groups of documents
  ...
  cmd
    ...
    .longDescription(`Filter the library to documents that have the given tag.  Tags
  are case-sensitive.`)
    .example('mendeley library by-tag machine-learning')
  ...
  cmd
    ...
    .longDescription(`Sort by \`created\` descending and return the first N
  ...
  slow; consider --limit to sample.`)
    ...
    .action(async (_args, flags, out) => {
      const session = await buildSession();
      const kwargs = { view: 'client', pageSize: 50 };
  ...
  cmd
    ...
    .longDescription(`Resolves a DOI to a catalog entry, then creates a new
  ...
  places the new document into a folder (--folder).`)
    ...
    .action(async ([doi], flags, out) => {
      ...
      if (flags.folder) {
        await session.post(`/folders/${flags.folder}/documents`, {
          data: JSON.stringify({ id: created.id }),
          headers: {
