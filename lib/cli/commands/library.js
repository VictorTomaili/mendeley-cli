...
import { collect, parseLimit, take } from '../output.js';
...
export function register(program) {
  ...
  documents.  Defaults to 10.`,
    ...
    .action(async (_args, flags, out) => {
      ...
      // Stop iteration as soon as we have enough documents — the server
      // sorts by created desc, so the first N are the most recent.
      // Avoids draining the whole library just to slice it (#131).
      const docs = await take(
        session.documents.iter({
          view: 'client',
          sort: 'created',
          order: 'desc',
  ...
  slow; consider --limit to sample.`,
    ...
    .action(async (_args, flags, out) => {
      ...
      // --limit caps the number of documents actually scanned: we stop
      // iteration as soon as the cap is reached instead of draining
      // every page in pages of `limit` (#131).
      const limit = flags.limit ? parseLimit(flags.limit) : null;
      const docs = await take(session.documents.iter({ view: 'tags', pageSize: 50 }), limit);
      const byType = {};
      const byYear = {};
      const tagCount = {};
      let total = 0;
