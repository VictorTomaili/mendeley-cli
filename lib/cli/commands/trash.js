...
export function register(program) {
  ...
  document is removed from the trash and cannot be restored.`,
    ...
    .example('mendeley trash delete abcdef12-3456-7890 --yes')
    .action(async ([id], flags, out) => {
      if (!flags.yes) {
        out.fail('Refusing to permanently delete trashed document without --yes.', 2);
        return;
      }
      const session = await buildSession();
      const doc = await session.trash.get(id);
  ...
  cleanup.`,
    ...
    .action(async (_args, flags, out) => {
      if (!flags.yes) {
        // Use out.fail() (not raw process.exit + stderr) so:
        //  - JSON mode emits the { ok: false, error: ... } envelope (#136)
        //  - the event loop can drain gracefully via the CliExitError
        //    sentinel (#90) instead of the mid-stack process.exit()
        out.fail('Refusing to empty the trash without --yes.', 2);
        return;
      }
      const session = await buildSession();
