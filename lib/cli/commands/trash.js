...
export function register(program) {
  const cmd = program
    ...
    .longDescription(`Documents moved to the trash with \`mendeley documents
  ...
  cmd
    ...
    .longDescription(`Enumerate all documents currently in the trash.  Use --all
  ...
  cmd
    ...
    .longDescription(`Move a document out of the trash and back into the user
  ...
  cmd
    ...
    .longDescription(`Permanently delete one document from the trash.  The
  ...
  cmd
    ...
    .longDescription(`Empty the trash.  This is destructive and cannot be undone —
  ...
  cleanup.`)
    ...
    .action(async (_args, flags, out) => {
      ...
      const session = await buildSession();
      const all = await collect(session.trash.iter({ view: 'core' }));
      let count = 0;
      for (const d of all) {
        await d.delete();
        count += 1;
