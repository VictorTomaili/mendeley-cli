...
export function register(program) {
  ...
  \`mendeley auth exchange <code>\` after visiting it manually.`)
    ...
    .action(async (_args, flags, out) => {
      ...
      const listener = await listenForCode(listenPort, flow.state);
      // Build the redirect URI from the port the listener actually bound to.
      const actualRedirectUri = `http://localhost:${listener.port}`;
      const mendeley2 = new Mendeley({
        clientId: creds.clientId,
        clientSecret: creds.clientSecret,
        redirectUri: actualRedirectUri,
        host: creds.host,
      });
      const flow2 = await mendeley2.startAuthorizationCodeFlowAsync({ usePkce: true });
      const loginUrl = flow2.getLoginUrl();
      ...
      const session = await flow2.authenticate(captured.code);
