...
function savePending(obj) {
  writePrivateJson(PENDING_PATH, obj);
}
 ...
 */
function readFromStdin(output = process.stdout) {
...
export function register(program) {
  ...
  (for end-users) and the client-credentials flow (for read-only
  catalog access — client-credentials tokens cannot read user-library
  resources such as documents, files, folders, annotations, or
  profiles).  The CLI uses the authorisation-code flow with PKCE
  ...
  auth
    ...
    .longDescription(
      `Start the interactive authorisation-code (PKCE) login flow.

  The CLI does NOT open a browser and does NOT run a local callback
  server.  It prints the authorisation URL, you complete the login
  in a browser yourself, then paste the resulting redirect URL back.

  Steps (printed in the terminal):
    1. Open the printed URL in a browser and complete the Mendeley
       login and authorisation.
    2. After you approve access, the browser redirects to a URL
       beginning with your redirect URI (http://localhost:…).
       That page may show "This site can't be reached" — that is
       normal; the CLI is not listening.
    ...
    .action(async (_args, flags, out) => {
      ...
      // Print clear, ordered browser-handoff instructions.  The CLI
      // does not open a browser or run a callback server, so we make
      // the manual copy/paste round-trip explicit (issue #53).
      const redirectHost = creds.redirectUri ? new URL(creds.redirectUri).host : 'localhost:…';
      const prompt = out.format === 'json' ? process.stderr : process.stdout;
      prompt.write('\n');
      prompt.write('Mendeley login — PKCE authorisation-code flow\n');
      prompt.write('\n');
      prompt.write('  Step 1  Open this URL in your browser and complete the\n');
      prompt.write('          Mendeley login and authorisation:\n');
      prompt.write('\n');
      prompt.write('            ' + loginUrl + '\n');
      prompt.write('\n');
      prompt.write(
        `  Step 2  After you approve access, the browser redirects to a\n` +
          `          URL starting with http://${redirectHost}.  That page\n` +
          `          may show "This site can't be reached" — that is\n` +
          `          normal; the CLI does not run a callback server.\n`,
      );
      prompt.write('\n');
      prompt.write('  Step 3  Copy the ENTIRE URL from the browser address bar.\n');
      prompt.write('\n');
      prompt.write('  Step 4  Paste it below:\n');
      prompt.write('\n');
      prompt.write(`  Redirect URL (http://${redirectHost}/…) > `);

      const rawRedirectUrl = await readFromStdin(prompt);
      prompt.write('\n');
  ...
  auth
    ...
    .longDescription(
      `Print a summary of the current configuration (use --format json
  for machine-parseable output; default is text).  Client secrets
  and access tokens are never printed in full — only booleans
  ...
  auth
    ...
    .longDescription(
      `Performs a live API call with the saved token.  Returns the
  profile (text by default; use --format json for JSON).  Exits
  non-zero if the token is missing, expired, or invalid.  This is
  the canonical "is the auth working?" check.`,
    )
    .example('mendeley auth whoami')
  ...
  preserved.`,
    ...
    .action(async ([key, value], _flags, out) => {
      if (!ALLOWED_CREDENTIAL_KEYS.includes(key)) {
        out.fail(
          `Unknown credential key: ${JSON.stringify(key)}. ` +
            `Allowed keys: ${ALLOWED_CREDENTIAL_KEYS.join(', ')}.`,
        );
        return;
      }
      // Read only the credentials file. Do NOT use loadCredentials() here:
      // that function merges in token.json tokens, which would then be
      // written back to credentials.json (issue #57).
      const creds = loadCredentialsFromFile();
      creds[key] = value;
      const path = process.env.MENDELEY_CONFIG || CONFIG_PATH;
      // sanitizeCredentials is belt-and-suspenders — strip any non-allowed
      // keys that may have been in the file (e.g. from older versions).
      writePrivateJson(path, sanitizeCredentials(creds));
  ...
  even if empty.`,
    ...
    .action(async ([key], _flags, out) => {
      ...
      // Read only the credentials file. See `auth set` for why this
      // differs from loadCredentials().
      const creds = loadCredentialsFromFile();
      delete creds[key];
      const path = process.env.MENDELEY_CONFIG || CONFIG_PATH;
      writePrivateJson(path, sanitizeCredentials(creds));
  ...
  previous \`mendeley auth login\` run).`,
    ...
    .action(async ([raw], flags, out) => {
      ...
      try {
        ...
        // Pass the raw input (URL or bare code) so the SDK can extract
        // and validate the state parameter from full redirect URLs.
        // Bare codes skip validation (documented escape hatch).
        const session = await flow.authenticate(raw);
        saveToken(session.token, flags.tokenFile);
