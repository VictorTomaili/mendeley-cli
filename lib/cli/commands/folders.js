...
export function register(program) {
  const cmd = program
    ...
    .longDescription(`Folders are user-defined collections of documents.  Each
  ...
  cmd
    ...
    .longDescription(`List folders in the user library.  Use --all to traverse
  ...
  cmd
    ...
    .longDescription(`Return the folder record: id, name, parent, group, and counts
  of documents.`)
  ...
  cmd
    ...
    .longDescription(`Create a new folder with the given name.  Pass --parent
  ...
  cmd
    ...
    .longDescription(`Update the name (and currently only the name) of an existing
  folder.`)
    .option('--name <name>')
  ...
  cmd
    ...
    .longDescription(`Delete a folder.  The documents inside it are not deleted —
  ...
  cmd
    ...
    .longDescription(`Return the documents in the given folder.  Use --format ids
  ...
  cmd
    ...
    .longDescription(`Insert an existing user document into a folder.  The document
  is not duplicated; it just gains a folder membership.`)
    .example('mendeley folders add-document <folderId> <docId>')
    .action(async ([folderId, docId], _flags, out) => {
      const session = await buildSession();
      const rsp = await session.post(`/folders/${folderId}/documents`, {
      ...
      out.write({ ok: rsp.ok, status: rsp.status, body: await rsp.text() });
    });

  cmd
    .command('remove-document <folderId> <docId>')
    .description('remove a document from a folder (does not delete the document)')
    .longDescription(`Removes a document from the given folder.  The document itself
  remains in the library.`)
    .example('mendeley folders remove-document <folderId> <docId>')
    .action(async ([folderId, docId], _flags, out) => {
      const session = await buildSession();
      const rsp = await session.delete(`/folders/${folderId}/documents/${docId}`);
      out.write({ ok: rsp.ok, status: rsp.status });
    });
}
