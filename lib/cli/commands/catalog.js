...
export function register(program) {
  ...
  Exactly one identifier flag is REQUIRED (the command fails
  immediately if zero or more than one are given):
    --doi <id>       DOI (e.g. 10.1038/nature12373)
    --arxiv <id>     arXiv id (e.g. 1706.03762)
    --isbn <id>      ISBN (10 or 13 digits)
    --issn <id>      ISSN
    --pmid <id>      PubMed id
    --scopus <id>    Scopus id
    ...
    .action(async (_args, flags, out) => {
      ...
      // Validate exactly one identifier before any network call (#126).
      const present = Object.entries(ids).filter(([, v]) => v !== undefined && v !== null);
      if (present.length === 0) {
        out.fail(
          'by-identifier requires exactly one of ' +
            '--doi/--arxiv/--isbn/--issn/--pmid/--scopus/--filehash.',
        );
        return;
      }
      if (present.length > 1) {
        out.fail(
          `by-identifier takes exactly one identifier, but received ${present.length} ` +
            `(${present.map(([k]) => '--' + k).join(', ')}). ` +
            `Identifier lookup is a single-identifier operation.`,
        );
        return;
      }
      const session = await buildSession();
      const doc = await session.catalog.byIdentifier({ ...ids, view: flags.view });
      out.write(doc);
    });
  ...
  catalog document and a confidence score between 0 and 100 (higher
  = more confident — the API defines 100 as very confident).`,
    )
    .option('--doi <id>')
    .option('--arxiv <id>')
    .option('--pmid <id>')
