...
export function register(program) {
  const cmd = program
    ...
    .longDescription(`Files are the binary blobs (typically PDFs) attached to
  ...
  \`mendeley files add-sticky-note\` to annotate.`)
    ...
    .example('mendeley files add-highlight <fileId> --positions \'[{"page":1,...}]\' --color \'{"r":255,"g":255,"b":0}\'');

  cmd
    .command('list')
    .description('list files (optionally scoped to a document/group/catalog)')
    .longDescription(`By default, lists files across the entire user library.  Use
  ...
  cmd
    ...
    .longDescription(`Return the metadata record for a file: id, file name, MIME
  ...
  the file content; use \`mendeley files download\` for that.`)
    ...
    .action(async ([id], _flags, out) => {
      ...
      if (!found) {
        // Fall back to fetching via the API.
        const rsp = await session.get(`/files/${id}`);
        out.write({ id, json: await rsp.json() });
      } else {
        out.write(found);
      }
    });
  ...
  cmd
    ...
    .longDescription(`Stream the file's bytes to \`<directory>/<fileId>\`.  Useful
  ...
  cmd
    ...
    .longDescription(`Delete a file from the library.  The parent document is left
  intact.`)
  ...
  cmd
    ...
    .longDescription(`Create a sticky-note annotation at the given (x, y) coordinates
  on a given page.  Coordinates are in points (PDF user space).`)
    .option('--text <text>')
    .option('--x <n>', null, undefined, parseFloat)
    .option('--y <n>', null, undefined, parseFloat)
    .option('--page <n>', null, undefined, parseInt)
    .example('mendeley files add-sticky-note <fileId> --text "Check this" --x 100 --y 200 --page 1')
    .action(async ([fileId], flags, out) => {
      const session = await buildSession();
      const rsp = await session.get(`/files/${fileId}`);
      const json = await rsp.json();
      const { File } = await import('../../../src/models/files.js');
      const file = new File(session, json);
      const ann = await file.addStickyNote(flags.text, flags.x, flags.y, flags.page);
      out.write(ann);
    });
  ...
  cmd
    ...
    .longDescription(`Create a highlight annotation spanning one or more bounding
  ...
  takes a JSON object with \`r\`, \`g\`, \`b\` integers in [0,255].`)
    ...
    .example('mendeley files add-highlight <fileId> --positions \'[{"top_left":{"x":50,"y":100},"bottom_right":{"x":500,"y":120},"page":1}]\'')
    .example('mendeley files add-highlight <fileId> --positions \'[...]\' --color \'{"r":255,"g":255,"b":0}\'')
    .action(async ([fileId], flags, out) => {
      const session = await buildSession();
      const rsp = await session.get(`/files/${fileId}`);
      const json = await rsp.json();
      ...
      const positions = JSON.parse(flags.positions || '[]').map((p) =>
        p instanceof BoundingBox ? p : BoundingBox.create(
          p.top_left, p.bottom_right, p.page
        )
      );
      const color = typeof flags.color === 'string' ? JSON.parse(flags.color) : flags.color;
      const colorObj = color && color.r !== undefined ? Color.create(color.r, color.g, color.b) : color;
