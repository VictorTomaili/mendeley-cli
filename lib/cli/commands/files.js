...
function parseBoundingBoxes(json, flagName = '--positions') {
  const raw = parseJson(json, flagName);
  if (!Array.isArray(raw) || raw.length === 0) {
    throw new Error(`${flagName} must be a non-empty JSON array of bounding boxes.`);
  }
  return raw.map((p, i) => {
    if (!p || typeof p !== 'object') {
      throw new Error(`${flagName}[${i}] must be an object.`);
    }
    if (!p.top_left || !p.bottom_right || p.page === undefined || p.page === null) {
      throw new Error(`${flagName}[${i}] requires top_left, bottom_right, and page.`);
    }
    return p;
  });
}
...
export function register(program) {
  ...
  the file content; use \`mendeley files download\` for that.`,
    ...
    .option('--document <id>', 'scope the lookup to a document (default: user library)')
    .option('--group <id>', 'scope the lookup to a group (default: user library)')
    .option('--catalog <id>', 'scope the lookup to the catalog (default: user library)')
    .example('mendeley files get abcdef12-3456-7890')
    .example('mendeley files get <id> --group 12345')
    .action(async ([id], flags, out) => {
      const session = await buildSession();
      const file = await findFileById(session, id, flags);
      if (!file) {
        out.fail(fileNotFoundMessage(id, flags));
        return;
      }
      out.write(file);
    });
  ...
  The output filename is chosen in this order:
    ...
    .option('--document <id>', 'scope the metadata lookup to a document (default: user library)')
    .option('--group <id>', 'scope the metadata lookup to a group (default: user library)')
    .option('--catalog <id>', 'scope the metadata lookup to the catalog (default: user library)')
    ...
    .action(async ([id, directory], flags, out) => {
      ...
      // not pass --filename. Falls back to the id if the file can't
      // be found. (#116) The metadata lookup traverses all pages and
      // respects the --document/--group/--catalog scope (#130).
      let defaultName = id;
      if (!flags.filename) {
        const meta = await findFileById(session, id, flags);
        // `displayName` resolves `filename` or the `file_name` alias
        // (#135). Falls back to the id if the file can't be found.
        if (meta && meta.displayName) defaultName = meta.displayName;
      }
      const rsp = await session.get(`/files/${encodePathSegment(id)}`, { stream: true });
  ...
  intact.`,
    ...
    .example('mendeley files delete abcdef12-3456-7890 --yes')
    .action(async ([id], flags, out) => {
      if (!flags.yes) {
        out.fail('Refusing to delete file without --yes.', 2);
        return;
      }
      const session = await buildSession();
      await session.delete(`/files/${encodePathSegment(id)}`);
  ...
  for you. \`--text\` carries the note text.`,
    ...
    .action(async ([fileId], flags, out) => {
      ...
      const session = await buildSession();
      const file = await findFileById(session, fileId, flags);
      if (!file) {
        out.fail(fileNotFoundMessage(fileId, flags));
        return;
      }
      ...
      let positions;
      if (flags.positions) {
        positions = parseBoundingBoxes(flags.positions, '--positions').map((p) =>
  ...
  takes a JSON object with \`r\`, \`g\`, \`b\` integers in [0,255].`,
    ...
    .action(async ([fileId], flags, out) => {
      // Validate --positions before any network call so the user gets
      // a clean error even when the API is unreachable (#191).
      if (!flags.positions) {
        out.fail('add-highlight requires --positions <json>.');
        return;
      }
      const { BoundingBox, Color } = await import('../../../src/models/common.js');
      let positions;
...
/**
 * Resolve the correct files accessor based on scope flags.
 *
 * `--document`, `--group`, and `--catalog` are mutually exclusive;
 * the first present flag wins. With none set, the user-library
 * accessor (`session.files`) is returned.
 */
function scopeForFiles(session, flags = {}) {
  if (flags.document) return session.documentFiles(flags.document);
  if (flags.group) return session.groupFiles(flags.group);
  if (flags.catalog) return session.catalogFiles(flags.catalog);
  return session.files;
}

/**
 * Build a helpful "file not found" message that mentions the active
 * scope so the user knows where the search was performed.
 */
function fileNotFoundMessage(id, flags = {}) {
  const where = flags.document
    ...
    : flags.group
      ...
      : flags.catalog
        ...
        : 'the user library';
  return (
    `File not found in ${where}: ${id}. The lookup now traverses every ` +
    'page; if the file truly does not exist, list files with ' +
    '`mendeley files list` to confirm the id.'
  );
}

/**
 * Look up a single file's metadata by id.
 *
 ...
 * JSON, so we can't fetch a single file directly. List the files in
 * the requested scope (user library by default, or a document/group/
 * catalog) and pick the matching id.
 *
 * The lookup traverses every page (#130) — files beyond the first
 * page are no longer reported as "not found". Iteration stops as soon
 * as the file is found, so it never fetches more pages than needed.
 *
 * Returns the `File` model instance, or `null` if no file matches.
 */
async function findFileById(session, fileId, flags = {}) {
  const files = scopeForFiles(session, flags);
  for await (const f of files.iter()) {
