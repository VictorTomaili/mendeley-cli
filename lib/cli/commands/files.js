...
export function register(program) {
  ...
  for you. \`--text\` carries the note text.`,
    ...
    .action(async ([fileId], flags, out) => {
      ...
      // Sticky notes share the annotation model with highlights; the
      // only API-level difference is the presence of `text` and that
      // positions are typically a single point. Reuse addStickyNote
      // when no color is given (its original shape), otherwise fall
      // through to a direct annotation create so color is honoured.
      if (!flags.color && !flags.positions) {
      ...
      // addHighlight doesn't set text; patch it onto the returned
      // annotation before printing.
      ann.json.text = flags.text;
      out.write(ann);
    });
