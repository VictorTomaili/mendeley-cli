...
export class Command {
   ...
   */
  command(name, desc) {
    // Parse `<arg>` and `[arg]` placeholders out of the name and
    // compute the base command name (everything outside the
    // placeholders) in a single pass. Reading the capture groups
    // directly and slicing between matches avoids the character-based
    // sanitization patterns CodeQL flags
    ...
    let lastIndex = 0;
    let base = '';
    while ((m = placeholder.exec(name)) !== null) {
      base += name.slice(lastIndex, m.index);
      lastIndex = m.index + m[0].length;
      if (m[1] !== undefined) {
        positional.push({ name: m[1], description: '', required: true });
      } else if (m[2] !== undefined) {
        positional.push({ name: m[2], description: '', required: false });
      }
    }
    base += name.slice(lastIndex);
