...
export function saveToken(token, path) {
  ...
  writePrivateJson(file, { saved_at: new Date().toISOString(), ...token });
  return file;
}
...
/** Write JSON with owner-only permissions where the platform supports it. */
export function writePrivateJson(file, value) {
  ...
  try {
    ...
    // Windows and some filesystems may not support POSIX permissions.
  }
}
